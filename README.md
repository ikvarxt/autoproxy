...
| 关于 AutoProxy x.y.z | 当前版本写在菜单项标题上，点开还有构建号、上次检查更新的时间，以及手动检查的入口 |
...
停止代理之前拔线，手机就被落在「代理指着一条不存在的隧道」的状态里 —— 这正是本工具存在的
理由，而人这时多半已经走了，不会去看菜单栏。所以这一刻会推一条系统通知：说清楚它现在上不了网、
手机上关不掉、插回这台 Mac 点「停止代理」才能恢复。

同一台手机只提醒一次。这状态会一直挂到线插回来，而探测每 3 秒一轮，每轮都响就成了骚扰；
换了台手机或换了端口才算新情况。

通知发不出去（没授权、或者程序不在应用程序文件夹里）时会退化成弹窗。宁可打扰人，
也好过让他对着一台上不了网的手机猜原因。
...
bundle id 和版本号，**然后等到不在抓包的时候才换上去**。

链路本身扛得住重启：隧道归 adb server 管，代理设置在手机上，两样都不随本进程走，所以换版本
不会掐断正在抓的包。等一等是为了另一件事 —— 重启那几秒没人盯着，这时候拔线不会被记录，
断链也没人补。等不及就点菜单里那一项。

等不到明天那一次的，从「关于」里手动查 —— 它跳过 24 小时的门槛，并且会明确区分「已经是最新的」
和「没问到 GitHub」：这两件事混在一起说，网络挂掉的人会以为自己手上就是最新版。
...
程序必须待在「应用程序」文件夹里。这不是整洁问题，是上面两个功能在别处都不成立：

| 位置 | 后果 |
|---|---|
| 从 zip 解压后直接双击 | 系统给的是一份隔离过的只读副本，自动更新**搬不动自己**，会一路静默失败 |
| 任何不在应用程序文件夹的位置 | 系统拒绝通知授权，连授权框都不弹（实测 `granted=false`），拔线提醒只能退化成弹窗 |

所以首次在别处启动时会弹一次引导：可以让它自己搬过去，也可以点「我自己拖」在访达里手动拖。
只读位置上这一提示不受「不用再提示」约束 —— 那不是偏好问题，是功能在那儿根本不成立。

搬运复用自动更新那条路径：拷一份到目标位置，等本进程退出后换上去再拉起来，源在原地可写时一并清掉，
免得下次双击到旧的那个。
